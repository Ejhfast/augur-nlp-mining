...
def run():
  ...
  for line1, line2 in pairwise(inp):
    ...
    if all(map(lambda actions: approve(actions), [actions1, actions2])):
      ...
      if nowhitelist or (not isTsv and (len(whitelist.intersection(verbobject1.split())) > 0 and len(whitelist.intersection(verbobject2.split()))> 0)) or (isTsv and whitelist.get(verbobject1) == True):
